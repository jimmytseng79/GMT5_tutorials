...
gmt gmtset PS_MEDIA=A3 FONT_LABEL=16p,1,black

rem -------- column 1 -------- 
set name=abyss
makecpt -C%name%.cpt -Z > tmp1.cpt
makecpt -C%name%.cpt -T-1/1/0.25 > tmp2.cpt
gmt psscale -Ctmp1.cpt -Dx0/37+w6/0.5+h+ml -Ba+l%name% -S -P -K ^
...
makecpt -C%name%.cpt -Z > tmp1.cpt
makecpt -C%name%.cpt -T-1/1/0.25 > tmp2.cpt
gmt psscale -Ctmp1.cpt -Dx16/4+w6/0.5+h+ml -Ba+l%name% -S -K -O ^
--FONT_ANNOT_PRIMARY=.1p >> %ps%
